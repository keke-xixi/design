...
func _on_item_gained(item_id: String, amount: int, rarity: String) -> void:
	...
	if rarity in ["rare", "epic", "legendary"]:
		prefix = "【%s】" % rarity
	_spawn(player.global_position + Vector2(0, -20), "%s%s x%d" % [prefix, name, amount], color)
...
func _on_damage(pos: Vector2, amount: int, is_player: bool) -> void:
	_spawn(pos, str(amount), Color(1.0, 0.45, 0.4) if is_player else Color(0.85, 0.95, 0.55))

func _on_attack(gain: int, _total: int) -> void:
	var player := get_tree().get_first_node_in_group("player") as Node2D
	if player:
		_spawn(player.global_position + Vector2(0, -24), "+%d 攻" % gain, Color(0.55, 0.95, 0.65))
...
func _on_breakthrough(realm_id: String) -> void:
	...
	_spawn(player.global_position + Vector2(0, -36), "突破 · %s" % name, Color(1.0, 0.88, 0.45))
